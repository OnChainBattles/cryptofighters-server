...
// Wallet-to-socket mapping for direct notifications
// Updated on every auth, cleared on disconnect
...
app.get('/lobbies', (req, res) => {
  res.json(lobbyManager.getOpenLobbies(io));
});
...
io.on('connection', (socket) => {
  ...
  socket.on('auth_with_token', (data) => {
    ...
    // Update wallet-to-socket mapping for direct notifications
    walletToSocket.set(walletAddress, socket.id);

    console.log(`[AUTH] Token auth OK for ${walletAddress.slice(0, 8)}...`);

    // Check for pending lobby and rejoin room (for opponent_joined/battle_start notifications)
    const pendingLobby = lobbyManager.getLobbyByWallet(walletAddress);
    if (pendingLobby && (pendingLobby.status === 'waiting' || pendingLobby.status === 'ready')) {
      socket.join(`lobby:${pendingLobby.id}`);
      // Update socket ID in lobby so events reach the right socket
      if (pendingLobby.creatorWallet === walletAddress) {
        pendingLobby.creatorSocketId = socket.id;
      } else if (pendingLobby.opponentWallet === walletAddress) {
        pendingLobby.opponentSocketId = socket.id;
      }
      console.log(`[AUTH] Rejoined lobby room: ${pendingLobby.id}`);
    }
  ...
  socket.on('auth_verify', (data) => {
    ...
    try {
      ...
      // Update wallet-to-socket mapping for direct notifications
      walletToSocket.set(socket.walletAddress, socket.id);
      ...
      // Check for pending lobby and rejoin room (for opponent_joined/battle_start notifications)
      const pendingLobby = lobbyManager.getLobbyByWallet(socket.walletAddress);
      if (pendingLobby && (pendingLobby.status === 'waiting' || pendingLobby.status === 'ready')) {
        socket.join(`lobby:${pendingLobby.id}`);
        // Update socket ID in lobby so events reach the right socket
        if (pendingLobby.creatorWallet === socket.walletAddress) {
          pendingLobby.creatorSocketId = socket.id;
        } else if (pendingLobby.opponentWallet === socket.walletAddress) {
          pendingLobby.opponentSocketId = socket.id;
        }
        console.log(`[AUTH] Rejoined lobby room: ${pendingLobby.id}`);
      }
  ...
  socket.on('create_lobby', (data) => {
    ...
    // Check if createLobby returned an error (wallet already has on-chain lobby)
    if (result.error) {
      console.log(`[LOBBY] REJECTED: ${result.error}`);
    ...
    io.emit('lobbies_updated', lobbyManager.getOpenLobbies(io));
  ...
  socket.on('get_lobbies', () => {
    socket.emit('lobbies_list', lobbyManager.getOpenLobbies(io));
  });
  ...
  socket.on('disconnect', (reason) => {
    ...
    if (socket.walletAddress) {
      // Only clear wallet-to-socket if this socket is the current one for this wallet
      // (prevents clearing if user reconnected with a new socket before old one timed out)
      if (walletToSocket.get(socket.walletAddress) === socket.id) {
